...
- Google 계정으로 가입 신청 → 관리자 승인 → 회원 전용 게시판 접근.
- 승인된 회원만 글·댓글을 읽고 작성. 작성자와 관리자만 수정·삭제. 공지 작성은 관리자만 가능.
- 관리자 회원 목록, 가입 승인, 이용 제한. 제한 시 기존 로그인도 즉시 해제.
...
1. 운영할 Google 계정으로 새 Google 스프레드시트를 만듭니다. 제목 예: `CIC 회원 게시판 데이터`.
...
첫 로그인은 `ADMIN_EMAILS`에 지정한 Google 계정으로 합니다. 이 계정은 관리자 및 승인된 회원으로 등록됩니다. 다른 회원은 처음 로그인하면 승인 대기가 됩니다. **회원 게시판 → 회원 승인 관리**에서 이름과 이메일을 확인해 승인합니다. Google 로그인 자체만으로 CIC 회원을 자동 승인하지 않습니다.

현재 로그인 세션은 브라우저 메모리에만 보관합니다. 새로고침하거나 창을 닫으면 다시 로그인해야 합니다. 서버 세션은 6시간 후 만료되고, 로그아웃 또는 이용 제한으로 폐기됩니다. 게시글·댓글 자체는 Google Sheets에 보관됩니다.

## 6. 실제 계정으로 확인할 항목

1. 비로그인 상태: 공개 가이드는 열리고 회원 게시글은 볼 수 없어야 합니다.
2. 일반 Google 계정 가입: 승인 대기 상태이며 회원 글/댓글을 읽거나 쓸 수 없어야 합니다.
3. 관리자 계정 승인 후 일반 계정에서 **승인 상태 확인**: 글·댓글 작성 및 새 조회에서 보존 확인.
