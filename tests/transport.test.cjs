...
function transport() {
  ...
  vm.runInNewContext(source,{window,document,location:{origin:'https://example.github.io'},CIC_CONFIG:{apiUrl:'https://script.google.com/macros/s/deployment/exec',googleClientId:'client.apps.googleusercontent.com'},crypto:webcrypto,URL,Uint8Array,Map,Promise,Error,setTimeout,clearTimeout});
  return {api:window.CIC_API,elements,get submitted(){return submitted;},emit:e=>listener(e)};
}
