...
최신 백엔드 배포는 Apps Script **버전 14**이다. 아래의 버전 11/원격 소스 불일치 내용은 해결 전 기록이며, 업로드 응답 오류는 버전 12에서 수정했다. 최신 UI 변경은 문서 끝의 기록을 참조한다.
...
- 정적 프런트엔드 배포 대상이며 Apps Script는 버전 14를 유지한다. 기존 GitHub Pages 절차로 배포하고 실제 HTTP 응답을 확인한다.
