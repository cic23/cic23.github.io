...
- Canva MCP 플러그인과 계정 재인증을 확인했다. `dist/index.html`은 JavaScript로 본문을 동적 렌더링하는 웹앱이므로 Canva 가져오기 결과는 코드·동작을 보존하는 편집본이 아니라 텍스트 위주의 정적 디자인임을 확인했다.
- CIC 소개에서 미확정 통계(창립 연도·활동 단원·누적 봉사시간), 해당 확인 안내, 선생님 기념사 게재 예정 안내를 제거했다. 커밋 `ef58f5d`를 Pages에 배포했고 HTTP 200을 확인했다.
- 지킴이 로그의 비로그인 안내에서 CIC 로고를 제거하고 제목을 `CIC 지킴이들과 이야기를 나눠요`로 변경했다. 한국어·영어 매핑을 함께 갱신했으며, 커밋 `546955d`를 Pages에 배포하고 HTTP 200을 확인했다. 두 변경 모두 `node --test tests/backend.test.cjs tests/transport.test.cjs` 및 `dist/*.js` 문법 검사를 통과했다.
...
1. **푸터 개선:** 현재 푸터의 정보 구조, 로고·연락처·SNS 링크, 여백과 PC·모바일 반응형을 검토하고 디자인을 개선한다. 원본의 디자인·텍스트·이미지를 보존한다는 프로젝트 원칙을 따른다.
2. 최초 관리자 계정과 다른 일반 Google 계정으로 PC·모바일 실사용 검증을 마친다. 일반 계정 자동 승인, 새로고침 뒤 로그인 복원, 지킴이 로그 글·댓글 작성, 로그아웃, 이용 제한 후 기존 세션 거부를 확인한다. 오류 시 Apps Script 실행 로그와 브라우저 콘솔만 확인하며 secret·세션·회원 데이터는 공유하거나 Git에 기록하지 않는다.
3. `https://soribook.github.io/soribook`를 직접 확인한 뒤 확인 가능한 범위만 참고한다. 접근 불가 또는 미확인 내용을 추정해 구현하지 않는다.
4. PC·모바일에서 한국어/English 토글을 실제로 검증한다. 깊은 링크(`?lang=en#guide/wolmi`), 페이지 이동·새로고침 뒤 언어 유지, 로그인 상태·작성 중 댓글 초안 보존, 이미지/CSS/JS 요청 오류를 확인한다.
5. 영문 원고의 고유 기관·행사명 표기가 공식 명칭과 일치하는지 검토한다. 한국어 원문·이미지·디자인을 보존하며, 게시글·댓글 원문 자동 번역과 LLM 기반 번역은 별도 단계로 남긴다.
