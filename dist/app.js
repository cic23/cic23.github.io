(() => {
  'use strict';
  const C = window.CIC_CONTENT, main = document.getElementById('main'), modal = document.getElementById('modal');
  const labels = { activity: '활동 기록', free: '자유 게시판', notice: '공지' };
  let member = null, epoch = 0, currentPost = null, currentComments = [], toastTimer, gisPromise;
  const esc = v => String(v ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const date = v => new Date(v).toLocaleDateString('ko-KR', {timeZone:'Asia/Seoul',year:'numeric',month:'long',day:'numeric'});
  ...
  function updateAccount() { document.getElementById('account-button').textContent = member ? `${member.name} 님` : '회원 로그인'; }
  function guideCards() { return C.places.map(p => `<article class="guide-card"><span class="number">${p.number}</span><p class="category">${p.category}</p><h3><a href="#guide/${p.id}">${esc(p.title)}</a></h3><p>${esc(p.short)}</p><a class="card-link" href="#guide/${p.id}">탐방 가이드 읽기 ↗</a></article>`).join(''); }
  function home() {
    const group = asset(CIC_CONFIG.assets.group);
    return `<section class="hero"><div class="hero-copy"><span class="eyebrow">CIC · Incheon Heritage Guide</span><h1>우리가 지키는 역사,<br>함께 이어갈 미래.</h1><p>${esc(C.subtitle)}</p><p class="small-text">채드윅송도국제학교 CIC와 함께하는<br>인천 문화유산 가이드</p><div class="button-row"><a class="button accent" href="#guide">문화유산 만나보기</a><a class="button secondary" href="#board">지킴이 로그</a></div></div><figure class="hero-photo"><img src="${esc(group || './assets/incheon-port.jpg')}" alt="${group ? '인천상륙작전기념관에서 함께한 CIC 단원들' : '자유공원에서 바라본 인천항의 과거 풍경'}"><figcaption>${group ? '인천상륙작전기념관 · CIC 현장 탐방' : 'INCHEON ARCHIVE · 자유공원에서 바라본 인천항<br><a href="https://commons.wikimedia.org/wiki/File:Port_of_Incheon.jpg" target="_blank" rel="noopener noreferrer">Davidinkorea · Public domain · 2006년 등록 사진</a>'}</figcaption></figure></section>
    <div class="value-strip">${C.values.map(v => `<a href="#values/${v.en.toLowerCase()}">${v.ko}<span>${v.en}</span></a>`).join('')}</div>
    ...
    <section class="wrap"><div class="section-heading"><div><span class="eyebrow">Our Practice</span><h2>현장에서 실천하는 가치</h2></div><a href="#values">활동 살펴보기 ↗</a></div><div class="grid-3">${C.achievements.slice(0,3).map((a,i) => `<article class="activity"><span class="eyebrow">0${i+1} / CIC 활동</span>${activityPhoto(a.asset,a.title)}<h3>${esc(a.title)}</h3><p>${esc(a.text)}</p></article>`).join('')}</div></section>`;
  }
  function about() {
    const stat = (value,label,suffix) => `<div><strong>${value == null ? '확인 중' : esc(value)+suffix}</strong><span>${label}</span></div>`;
    const reflections = C.reflections.filter(x => x.text.trim());
    return title('About CIC','작은 관심에서 시작된, 진정성 있는 실천', '채드윅송도국제학교 청소년 국가유산지킴이') + `<div class="reading"><span class="eyebrow">발간사</span><h2>우리가 서 있는 이 땅의 역사를<br>미래로 연결합니다</h2>${C.foreword.map(p=>`<p>${esc(p)}</p>`).join('')}<h2>CIC를 소개합니다</h2><p>${esc(C.introduction)}</p><div class="statistics">${stat(C.foundingYear,'창립 연도','년')}${stat(C.memberCount,'활동 단원','명')}${stat(C.volunteerHours,'누적 봉사시간','시간')}</div><p class="muted small-text">창립 연도와 단원 수는 클럽 기록, 누적 봉사시간은 1365 자원봉사포털 기록을 확인한 뒤 반영합니다.</p><div class="notice">선생님의 기념사는 담당 선생님과 원고를 확인한 뒤 게재할 예정입니다.</div></div><section class="wrap tinted"><div class="section-heading"><div><span class="eyebrow">Our Journey</span><h2>함께 쌓아온 활동</h2></div></div><div class="activity-grid">${C.achievements.map(a=>`<article class="activity">${activityPhoto(a.asset,a.title)}<h3>${esc(a.title)}</h3><p>${esc(a.text)}</p></article>`).join('')}</div><p class="source" style="margin-top:28px">제공된 CIC 발간 원고를 바탕으로 정리했습니다. 수상·위촉 및 언론 게재의 정확한 명칭과 시기는 확인 후 보완합니다.</p></section><section class="reading"><span class="eyebrow">지킴이 로그</span><h2>우리가 문화유산에 ‘푹’ 빠진 이유</h2>${reflections.length ? reflections.map(r=>`<blockquote class="quote-block">${esc(r.text)}<p class="muted small-text">${esc(r.name)} · ${r.grade}학년</p></blockquote>`).join('') : '<p class="muted">단원들의 소감을 모으고 있습니다. 직접 경험하고 느낀 이야기를 곧 이곳에서 전하겠습니다.</p>'}</section>`;
  }
  function values() { return title('Five Core Values','다섯 가지 가치, 다섯 가지 실천', '존중 · 책임감 · 정직 · 공정 · 배려') + `<div class="wrap">${C.values.map((v,i)=>`<section id="value-${v.en.toLowerCase()}" class="value-section"><div><span class="value-index">0${i+1}</span><h2>${v.ko}</h2><span class="english">${v.en}</span></div><div class="value-body"><blockquote>“${esc(v.quote)}”</blockquote>${activityPhoto(v.asset,v.ko+' 활동')}${v.activities.map(([h,p])=>`<h3>${esc(h)}</h3><p>${esc(p)}</p>`).join('')}</div></section>`).join('')}</div>`; }
  ...
  function lockedBoard() {
    ...
    if (!CIC_API.configured()) { heading='지킴이 로그를 준비하고 있습니다'; message='CIC 소개와 문화유산 가이드는 지금 둘러볼 수 있습니다. 회원 서비스가 연결되면 Google 로그인 후 글과 댓글을 나눌 수 있습니다.'; button='<a class="button secondary" href="#guide">문화유산 가이드 보기</a>'; }
    else if (!member) { heading='CIC 지킴이들과 이야기를 나누세요'; message='Google 계정으로 로그인하면 바로 활동 기록과 댓글을 읽고 작성할 수 있습니다.'; button='<button class="button" data-action="login">Google 계정으로 로그인</button>'; }
  ...
  async function board(page, stamp) {
    main.innerHTML = title('Community','함께 기록하는 CIC', '활동의 순간과 생각을 회원들과 나눕니다.') + '<section class="board-shell" id="board-content"></section>';
    ...
    target.innerHTML='<p role="status">게시글을 불러오고 있습니다…</p>';
    const data = await CIC_API.request('listPosts',{page}); if (stamp !== epoch) return;
    target.innerHTML=`<div class="board-toolbar"><h2>지킴이 로그 <span class="muted small-text">${data.total}개의 글</span></h2><div class="button-row" style="margin:0">${member.role==='admin'?'<a class="button secondary small" href="#members">회원 관리</a>':''}<button class="button small" data-action="new-post">글쓰기</button></div></div>${data.posts.length ? `<div class="post-list">${data.posts.map(p=>`<a class="post-row" href="#post/${p.id}"><span class="post-tag">${esc(labels[p.category])}</span><div><h3>${esc(p.title)}</h3><p>${esc(p.authorName)} · ${date(p.createdAt)}</p></div><span class="count">댓글 ${p.commentCount}</span></a>`).join('')}</div>` : '<div class="empty"><h3>아직 등록된 글이 없습니다</h3><p>첫 번째 CIC 활동 이야기를 남겨주세요.</p></div>'}${pagination(page,data.pages,'board')}`;
  }
  function pagination(page,pages,route) { return pages>1 ? `<div class="paging">${page>1?`<a class="button secondary small" href="#${route}/${page-1}">이전</a>`:''}<span>${page} / ${pages}</span>${page<pages?`<a class="button secondary small" href="#${route}/${page+1}">다음</a>`:''}</div>` : ''; }
  async function post(id, commentPage, stamp) {
    if (!member || member.status !== 'approved') { await board(1,stamp); return; }
    main.innerHTML=title('Community','지킴이 로그')+'<div class="reading"><p role="status">게시글을 불러오고 있습니다…</p></div>';
    ...
    main.innerHTML=`<article class="reading"><a href="#board" class="muted small-text">← 게시판 목록</a><span class="eyebrow" style="margin-top:35px">${esc(labels[p.category])}</span><h1 style="font-size:2rem">${esc(p.title)}</h1><div class="post-meta"><span>${esc(p.authorName)}</span><time>${date(p.createdAt)}</time>${p.version>1?'<span>수정됨</span>':''}</div>${canEdit(p.authorId)?'<div class="button-row"><button class="text-button" data-action="edit-post">수정</button><button class="text-button danger" data-action="delete-post">삭제</button></div>':''}<div class="post-body">${esc(p.body)}</div><section class="comments"><h2>댓글 <span class="muted small-text">${data.commentCount}</span></h2>${data.comments.length ? data.comments.map(c=>`<article class="comment"><div class="comment-head"><strong>${esc(c.authorName)}</strong><time>${date(c.createdAt)}</time>${c.version>1?'<span class="muted">수정됨</span>':''}${canEdit(c.authorId)?`<span class="comment-actions"><button class="text-button" data-action="edit-comment" data-id="${c.id}">수정</button><button class="text-button danger" data-action="delete-comment" data-id="${c.id}">삭제</button></span>`:''}</div><p>${esc(c.body)}</p></article>`).join('') : '<p class="muted">첫 번째 댓글을 남겨주세요.</p>'}${pagination(commentPage,data.commentPages,'post/'+id)}<form id="comment-form"><div class="field"><label for="comment-body">댓글 쓰기</label><textarea id="comment-body" name="body" required maxlength="2000" placeholder="활동에 대한 생각을 나눠주세요."></textarea></div><p class="inline-error" role="alert"></p><button class="button" type="submit">댓글 등록</button></form></section></article>`;
    const form=document.getElementById('comment-form'); let mutationId=crypto.randomUUID();
    form.addEventListener('submit', async e => {e.preventDefault(); await submit(form,async()=>{await CIC_API.request('createComment',{postId:p.id,body:form.elements.body.value,mutationId}); mutationId=crypto.randomUUID(); toast('댓글을 등록했습니다.'); await route();});});
  }
  ...
  async function route() {
    ...
    const titles={home:'인천 문화유산 가이드',about:'CIC 소개',values:'다섯 가지 가치',guide:'인천 문화유산',board:'지킴이 로그',post:'지킴이 로그',members:'회원 관리'};
    ...
    try {
      ...
      main.innerHTML=title('Community','페이지를 불러오지 못했습니다')+`<div class="reading"><p class="inline-error" role="alert">${esc(e.message)}</p><div class="button-row"><button class="button secondary" data-action="retry">다시 확인</button><a class="button secondary" href="#board">게시판으로</a></div></div>`;
    }
  }
  async function submit(form,fn) {
    ...
    try {await fn();}catch(e){err.textContent=e.message;}finally{button.disabled=false;}
  }
  function editor(p=null) {
    const mutationId=crypto.randomUUID();
    openModal(`<h2 id="modal-title">${p?'게시글 수정':'새로운 이야기'}</h2><form id="post-form"><div class="field"><label for="post-category">분류</label><select id="post-category" name="category">${Object.entries(labels).filter(([k])=>k!=='notice'||member.role==='admin').map(([k,v])=>`<option value="${k}" ${p?.category===k?'selected':''}>${v}</option>`).join('')}</select></div><div class="field"><label for="post-title">제목</label><input id="post-title" name="title" required maxlength="120" value="${esc(p?.title||'')}" placeholder="어떤 이야기를 나누고 싶으신가요?"></div><div class="field"><label for="post-body">내용</label><textarea id="post-body" name="body" required maxlength="10000" rows="9">${esc(p?.body||'')}</textarea></div><p class="muted small-text">승인된 CIC 회원에게만 공개됩니다.</p><p class="inline-error" role="alert"></p><button class="button" type="submit">${p?'수정 저장':'게시글 등록'}</button></form>`);
    const form=document.getElementById('post-form');
    form.addEventListener('submit',e=>{e.preventDefault();submit(form,async()=>{const data=Object.fromEntries(new FormData(form));data.mutationId=mutationId;if(p){data.id=p.id;data.version=p.version;}const result=await CIC_API.request(p?'updatePost':'createPost',data);modal.close();toast(p?'게시글을 수정했습니다.':'게시글을 등록했습니다.');if(location.hash==='#post/'+result.post.id)await route();else location.hash='post/'+result.post.id;});});
  }
  function editComment(c) {
    openModal(`<h2 id="modal-title">댓글 수정</h2><form id="edit-comment-form"><div class="field"><label for="edit-comment-body">내용</label><textarea id="edit-comment-body" name="body" required maxlength="2000">${esc(c.body)}</textarea></div><p class="inline-error" role="alert"></p><button type="submit" class="button">수정 저장</button></form>`);
    const f=document.getElementById('edit-comment-form');f.addEventListener('submit',e=>{e.preventDefault();submit(f,async()=>{await CIC_API.request('updateComment',{id:c.id,version:c.version,body:f.elements.body.value});modal.close();toast('댓글을 수정했습니다.');await route();});});
  }
  function confirmAction(heading,message,fn) {
    openModal(`<h2 id="modal-title">${esc(heading)}</h2><p>${esc(message)}</p><form id="confirm-form"><p class="inline-error" role="alert"></p><div class="button-row"><button type="submit" class="button accent">확인</button><button type="button" class="button secondary" data-action="close">취소</button></div></form>`);
  ...
  function loadGis() {
    ...
    gisPromise=new Promise((resolve,reject)=>{const s=document.createElement('script');s.src='https://accounts.google.com/gsi/client';s.async=true;const timer=setTimeout(()=>{gisPromise=null;reject(new Error('Google 로그인 화면을 불러오지 못했습니다. 인터넷 연결을 확인해주세요.'));},15000);s.onload=()=>{clearTimeout(timer);resolve();};s.onerror=()=>{clearTimeout(timer);gisPromise=null;reject(new Error('Google 로그인 연결을 확인해주세요.'));};document.head.append(s);});return gisPromise;
  }
  async function loginDialog() {
    ...
    openModal('<h2 id="modal-title">CIC에 오신 것을 환영합니다</h2><p>Google 계정으로 로그인하면 바로 지킴이 로그를 이용할 수 있습니다.</p><p class="muted small-text">회원 확인을 위해 Google 계정의 이름, 이메일, 계정 식별자를 저장합니다. 게시글에는 이름이 표시되며 이메일은 관리자만 확인할 수 있습니다. 계정 삭제는 관리자에게 요청해주세요.</p><button id="google-login" class="button secondary" disabled>Google 로그인 준비 중…</button><p class="inline-error" id="login-error" role="alert"></p>');
    ...
    }catch(e){if(error.isConnected){error.textContent=e.message;button.textContent='다시 시도';button.disabled=false;button.onclick=loginDialog;}}
  }
  document.addEventListener('click',async event=>{
    ...
    try{
      ...
      else if(action==='delete-post'&&currentPost){const p=currentPost;confirmAction('게시글을 삭제할까요?','이 글과 댓글은 지킴이 로그에서 더 이상 보이지 않습니다.',async()=>{await CIC_API.request('deletePost',{id:p.id,version:p.version});toast('게시글을 삭제했습니다.');location.hash='board';});}
      else if(action==='edit-comment'){const c=currentComments.find(c=>c.id===b.dataset.id);if(c)editComment(c);}
      else if(action==='delete-comment'){const c=currentComments.find(c=>c.id===b.dataset.id);if(c)confirmAction('댓글을 삭제할까요?','이 댓글은 더 이상 표시되지 않습니다.',async()=>{await CIC_API.request('deleteComment',{id:c.id,version:c.version});toast('댓글을 삭제했습니다.');await route();});}
      else if(action==='member-status'){const{id,status}=b.dataset;confirmAction(status==='approved'?'이 회원을 승인할까요?':'이 회원의 이용을 제한할까요?',status==='approved'?'지킴이 로그의 글과 댓글을 읽고 작성할 수 있게 됩니다.':'기존 로그인도 만료되며, 지킴이 로그를 이용할 수 없게 됩니다.',async()=>{await CIC_API.request('setMemberStatus',{id,status});await route();});}
      else if(action==='refresh-member'){b.disabled=true;const d=await CIC_API.request('me');member=d.member;updateAccount();await route();}
      else if(action==='logout'){b.disabled=true;try{await CIC_API.request('logout');CIC_API.clear();member=null;updateAccount();modal.close();await route();}catch(e){document.getElementById('logout-error').textContent=e.message;b.disabled=false;}}
  ...
  if(logo){
    const makeLogo=()=>{const el=document.createElement('img');el.src=logo;el.alt='CIC 로고';el.width=1500;el.height=1500;return el;};
