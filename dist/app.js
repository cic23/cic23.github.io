(() => {
  ...
  function home() {
    ...
    return html`<section class="hero"><div class="hero-copy"><span class="eyebrow">CIC · Incheon Heritage Guide</span><h1>우리가 지키는 역사,<br>함께 이어갈 미래.</h1><p>${esc(C.subtitle)}</p><p class="small-text">채드윅송도국제학교 CIC와 함께하는<br>인천 문화유산 가이드</p><div class="button-row"><a class="button accent" href="#guide">문화유산 만나보기</a><a class="button secondary" href="#board">지킴이 로그</a></div></div><figure class="hero-photo"><img src="${esc(group || './assets/incheon-port.jpg')}" alt="${group ? t('인천상륙작전기념관에서 함께한 CIC 단원들') : t('자유공원에서 바라본 인천항의 과거 풍경')}"><figcaption>${group ? t('인천상륙작전기념관 · CIC 현장 탐방') : t('INCHEON ARCHIVE · 자유공원에서 바라본 인천항<br><a href="https://commons.wikimedia.org/wiki/File:Port_of_Incheon.jpg" target="_blank" rel="noopener noreferrer">Davidinkorea · Public domain · 2006년 등록 사진</a>')}</figcaption></figure></section>
    <div class="value-strip">${C.values.map(v => `<a href="#values/${v.en.toLowerCase()}">${CIC_I18N.language === 'en' ? v.en : v.ko}<span>${v.en}</span></a>`).join('')}</div>
