(() => {
  ...
  async function post(id, commentPage, stamp) {
    ...
    main.innerHTML=title('Community','회원 게시글')+'<div class="reading"><p role="status">게시글을 불러오고 있습니다…</p></div>';
  ...
  async function route() {
    ...
    const titles={home:'인천 문화유산 가이드',about:'CIC 소개',values:'다섯 가지 가치',guide:'인천 문화유산',board:'회원 게시판',post:'회원 게시글',members:'회원 승인 관리'};
  ...
  document.addEventListener('click',async event=>{
    ...
    try{
      ...
      else if(action==='delete-post'&&currentPost){const p=currentPost;confirmAction('게시글을 삭제할까요?','이 글과 댓글은 회원 게시판에서 더 이상 보이지 않습니다.',async()=>{await CIC_API.request('deletePost',{id:p.id,version:p.version});toast('게시글을 삭제했습니다.');location.hash='board';});}
      ...
      else if(action==='member-status'){const{id,status}=b.dataset;confirmAction(status==='approved'?'이 회원을 승인할까요?':'이 회원의 이용을 제한할까요?',status==='approved'?'회원 게시글과 댓글을 읽고 작성할 수 있게 됩니다.':'기존 로그인도 만료되며, 회원 게시판을 이용할 수 없게 됩니다.',async()=>{await CIC_API.request('setMemberStatus',{id,status});await route();});}
